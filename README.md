...
- **Fix adaptive gradient clipping in layer mode, was incorrectly amplifying gradients smaller than the parameter, not clipping ones larger.**
  ...
  - Weight decay will likely need to be decreased and retuned. Recommend starting at 1e-6 if using stable weight decay, if not using swd, can start at 5e-4 or similar most likely.
  ...
  - LRs might need to be increased. Perhaps 1.5x, or more, depending on other parameters.
- Adjust all eps to be float32 tiny by default, or if eps value set to 0
ADOPTAOSchedule
- Adjust to apply clipping on step one
